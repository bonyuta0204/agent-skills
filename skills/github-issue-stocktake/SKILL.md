...
description: GitHub Issueの棚卸しをAIが実行する。既存Issueを5件前後のバッチで調査し、分類（ステータス更新）、担当領域の切り分け、重複統合、一次調査メモ（根拠・差分・方針）を作る。AIで直せるものは改修エージェントに渡す前提だが、改修PRの作成は別エージェントで行う。次のボールはPRのlabelで管理し、クローズ意思決定は人間が行う。
---
...
- Stocktake AIはIssueをクローズしない。クローズ可否の最終意思決定は人間が行う。

## Inputs
1. 対象リポジトリ・対象Issue番号（または範囲）
2. バッチサイズ（デフォルト5件）
3. 実行可能操作（Issue本文編集・ラベル付与・アサイン・重複リンク）
...
- 必要ならラベル・アサインを更新する。
- クローズ可判定（`クローズ可（解消）` / `クローズ可（重複）`）でも、Issueはクローズしない。
...
## Decision Rules
- 重複判定（`CLOSE_DUPLICATE`）時は必ず集約先Issue番号を明記する（例: `#123`）。
- 「直っている」判定は現行実装で根拠を確認してから行う。
- 「仕様変更でクローズ可判定」は変更後仕様の根拠（参照URL）を添える。
- 仕様書/画面項目定義を参照した場合は、AI_STOCKTAKE内にNotionページURLを必ず明記する。
- 根拠不足時は無理にクローズ可判定にせず、調査結果と不足点を残す。
